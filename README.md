...
- нативный и продвинутый режимы маскировки с локальными сайтами-заглушками;
...
| --- | --- |
| `OCSERV_CAMOUFLAGE=0\|1` | Отключить маскировку или включить нативный режим маскировки |
...
После включения обычного режима маскировки установщик может включить продвинутый TCP-only режим и предложит выбрать заглушку. Отдельный Nginx-контейнер `ocserv-camouflage-site` занимает публичный TCP/443 через host networking: TLS-клиенты с ALPN `h2` или `http/1.1` получают выбранный сайт, а соединения с другим или отсутствующим ALPN передаются контейнеру `ocserv-vps` на `127.0.0.1:8443`. TLS до ocserv не завершается на Nginx, реальный IP клиента передаётся через PROXY protocol, а секрет из URL продолжает проверять сам ocserv.
...
На существующей установке новая версия сгенерированной конфигурации Nginx применяется транзакционно при следующем обновлении образа VPN командой `ocserv-vps update`: перед активацией конфигурация проверяется, а при ошибке восстанавливается предыдущий файл. `update-manager` заменяет только файлы менеджера и сам по себе работающий стек не перезапускает.
...
Маршрутизация основана только на открытых полях TLS ClientHello: правильный SNI с ALPN `h2` или `http/1.1` направляется на сайт, а другой или отсутствующий ALPN — в ocserv. Неизвестный SNI всегда направляется на сайт. VPN-клиент, который сам рекламирует `h2` или `http/1.1`, в этом режиме попадёт на заглушку и не подключится, поэтому совместимость нужно проверить используемой версией клиента. Специально сформированный probe с правильным SNI и без браузерного ALPN всё ещё может обнаружить нативный ответ ocserv (404/401): схема закрывает лёгкую проверку современным HTTP/1.1-клиентом, рекламирующим этот ALPN, но не защищает от активного анализа.
...
Панель показывает состояние сервера и режима маскировки, управляет пользователями, завершает выбранные VPN-сессии, выводит журналы контейнеров и позволяет безопасно редактировать `ocserv.conf`.
...
![Параметры продвинутого режима маскировки](docs/images/ui-camouflage.png)

Раздел показывает публичную точку входа, состояние TCP и UDP/DTLS, маршрут через Nginx и выбранный источник сайта-заглушки. Секрет режима маскировки раскрывается только отдельным защищённым запросом, который записывается в журнал аудита UI.
